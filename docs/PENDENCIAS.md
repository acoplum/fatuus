...
| FAT-10 | **Bloqueia o próximo deploy no Cloud Run:** CORS do `AgentOS` (`allow_credentials=True`, allowlist incluindo `agno.com`/`localhost:3000`) permite leitura cross-origin autenticada — a Camada 2 transforma isso de config inerte em vetor real, porque agora existe sessão de navegador com Basic Auth em cache. Fix: `AgentOS(..., cors_allowed_origins=[<origem real do deploy>])`, que substitui a allowlist default, mais um teste travando que origens não listadas deixam de receber `Access-Control-Allow-Origin`. Achado na revisão final de 2026-09-02, não corrigido nesta rodada por depender da origem real do deploy. | aberta | 9 |
